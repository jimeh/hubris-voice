...
const temporary = () => {
  const directory = mkdtempSync(join(tmpdir(), "hubris-vendor-test-"));
...
const put = (file: string, data: string | Uint8Array) => {
...
async function fixture(extra: Record<string, string> = {}) {
  ...
  const files: Record<string, string | Uint8Array> = {
    ...
    "Sources/build.swift": "original\n",
    "Sources/removed.txt": "old\n",
    "Sources/data.bin": new Uint8Array([0, 1, 2]),
    "build.rs": "original\n",
    "removed.txt": "old\n",
    "data.bin": new Uint8Array([0, 1, 2]),
    LICENSE: "license\n",
    "not-selected.txt": "not vendored\n",
    ...extra,
  };
  ...
  const vendor = join(root, source.destination);
  await extract(source, archive, vendor);
...
for (const replacement of ["symlink", "regular file"] as const) {
  test(`tree hashing rejects a regular file replaced by a ${replacement} before opening`, () => {
    ...
    try {
      expect(() => treeEntries(tree)).toThrow(
        replacement === "symlink" ? "ELOOP" : "vendor file changed",
      );
...
test("archive verification rejects symlinks and non-regular files", async () => {
  ...
  const link = join(f.root, "link.tar.gz");
  symlinkSync(f.archive, link);
  await expect(extract(f.source, link, temporary())).rejects.toThrow();
  await expect(extract(f.source, f.root, temporary())).rejects.toThrow(
    "not regular",
  );
  const fifo = join(f.root, "fifo.tar.gz");
  expect(Bun.spawnSync(["mkfifo", fifo]).exitCode).toBe(0);
  await expect(extract(f.source, fifo, temporary())).rejects.toThrow(
    "not regular",
  );
});
...
test("extraction uses the verified bytes when the archive path is replaced after reading", async () => {
  ...
  const replacement = await fixture({
    "Sources/build.swift": "unverified replacement\n",
  });
  ...
  try {
    await extract(f.source, f.archive, destination);
  } finally {
    read.mockRestore();
  }
  expect(replaced).toBe(true);
  expect(readFileSync(join(destination, "Sources/build.swift"), "utf8")).toBe(
    "original\n",
  );
});

test("extraction preserves the declared inventory and excludes undeclared archive paths", async () => {
  const f = await fixture();
  const extracted = temporary();
  await extract(f.source, f.archive, extracted);
  expect(readFileSync(join(extracted, "LICENSE"), "utf8")).toBe("license\n");
  expect(existsSync(join(extracted, "not-selected.txt"))).toBe(false);
  await expect(
    extract(
      { ...f.source, paths: [...f.source.paths, "missing"] },
      f.archive,
      temporary(),
    ),
  ).rejects.toThrow("selected archive path is missing");
});

test("verification ignores only SwiftPM's local build directory", async () => {
  const f = await fixture();
  put(join(f.vendor, ".build/.git/config"), "generated build metadata\n");
  await expect(reproduce(f.source, f.root, f.archive)).resolves.toBeUndefined();
  put(join(f.vendor, "unknown-root/file"), "unrecorded source\n");
...
test("patch generation reproduces edits, additions, deletions, binary data, and executable files", async () => {
  ...
  rmSync(join(f.vendor, "Sources/removed.txt"));
  put(join(f.vendor, "Sources/data.bin"), new Uint8Array([0, 7, 2, 8]));
  chmodSync(join(f.vendor, "Sources/build.swift"), 0o755);
...
for (const change of ["modified", "added", "removed", "executable"] as const) {
  test(`verification rejects ${change} drift without modifying source or patches`, async () => {
    const f = await fixture();
    await refresh(f);
    const target = join(f.vendor, "Sources/build.swift");
    if (change === "modified") put(target, "unrecorded\n");
    if (change === "added") put(join(f.vendor, ".extra"), "unrecorded\n");
    if (change === "removed") rmSync(join(f.vendor, "Sources/removed.txt"));
    if (change === "executable") chmodSync(target, 0o755);
    const before = treeEntries(f.vendor);
    await expect(reproduce(f.source, f.root, f.archive)).rejects.toThrow(
      "differs from archive + patches",
    );
    expect(treeEntries(f.vendor)).toEqual(before);
    expect(
      readFileSync(join(f.root, f.source.patches[0]!.file), "utf8"),
    ).toBe("");
  });
}
...
test("stale patch fails instead of silently accepting the vendor tree", async () => {
  ...
  const patch = await refresh(f);
  put(
    join(f.root, f.source.patches[0]!.file),
    patch.replace("-original", "-not-the-original"),
  );
  await expect(reproduce(f.source, f.root, f.archive)).rejects.toThrow(
    "git apply failed",
  );
  expect(readFileSync(join(f.vendor, "Sources/build.swift"), "utf8")).toBe(
    "patched\n",
  );
});

test("checksum and selected path identity must match before accepting patches", async () => {
  const f = await fixture();
  await expect(
    reproduce({ ...f.source, sha256: "0".repeat(64) }, f.root, f.archive),
  ...
  rmSync(join(f.vendor, "Package.swift"));
  await expect(reproduce(f.source, f.root, f.archive)).rejects.toThrow();
});
...
test("invalid archive roots and traversal are rejected", async () => {
  ...
  for (const name of ["wrong-root/file", "../../escaped"]) {
    await Bun.write(
      f.archive,
      new Bun.Archive({ [name]: "bad" }, { compress: "gzip" }),
    );
...
test("downloads are verified, cached, and never silently replace corrupt cache entries", async () => {
  ...
  const source = {
    ...f.source,
    url: `http://127.0.0.1:${server.port}/source.tar.gz`,
  };
  const cache = join(f.root, "cache");
  try {
    const archive = await archiveFor(source, cache);
    expect(readFileSync(archive)).toEqual(readFileSync(f.archive));
    expect(await archiveFor(source, cache)).toBe(archive);
    expect(requests).toBe(1);
    writeFileSync(archive, "corrupt");
    await expect(archiveFor(source, cache)).rejects.toThrow(
      "checksum mismatch",
    );
    expect(requests).toBe(1);
    await expect(
      archiveFor({ ...source, sha256: "0".repeat(64) }, cache),
    ).rejects.toThrow("checksum mismatch");
    expect(existsSync(join(cache, `${"0".repeat(64)}.tar.gz`))).toBe(false);
  } finally {
    server.stop(true);
  }
});

test("manifest rejects ambiguous sources, unsafe paths, and unpinned URLs", async () => {
  const f = await fixture();
  ...
  for (const sources of [
    ...
    [
      {
        ...f.source,
        patches: [{ ...f.source.patches[0], file: "../outside.patch" }],
      },
    ],
    [{ ...f.source, destination: "../../outside" }],
    [{ ...f.source, paths: ["Sources", "Sources/Nested"] }],
    [{ ...f.source, archiveRoot: "../wrong" }],
    [{ ...f.source, url: "https://unrecorded.example/archive" }],
  ]) {
    put(file, JSON.stringify({ schema: 2, sources }));
...
test("index guard rejects vendor-only worktree and untracked drift", () => {
  const root = temporary();
  ...
  const command = (
    ...
    const result = Bun.spawnSync(["git", ...args], {
      cwd,
      env: isolatedGitEnvironment(cwd, inherited),
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  put(join(parent, "sentinel"), "parent index\n");
  command(parent, ["init", "--quiet", "--template="]);
  command(parent, ["add", "--all"]);
  command(parent, [
    "-c",
  ...
  const parentHead = command(parent, ["rev-parse", "HEAD"]);
  const parentIndex = readFileSync(join(parent, ".git/index"));
  ...
  command(root, ["init", "--quiet", "--template="], hostileHookEnvironment);
  command(root, ["add", "--all"], hostileHookEnvironment);
  command(
    root,
    [
      "-c",
  ...
  ).toThrow("Vendor/FluidAudio/source.swift");
  command(root, ["add", "Vendor/FluidAudio/source.swift"], hostileHookEnvironment);
  assertVendorIndexMatchesWorktree(root, ["Vendor", "third-party/vendor"], null);

  put(join(root, "Vendor/FluidAudio/source.swift"), "unstaged again\n");
  expect(() =>
    assertVendorIndexMatchesWorktree(
      root,
      ["Vendor", "third-party/vendor"],
  ...
  put(join(root, "third-party/vendor/new.patch"), "untracked\n");
  expect(() =>
    assertVendorIndexMatchesWorktree(
      root,
      ["Vendor", "third-party/vendor"],
