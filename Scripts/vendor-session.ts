...
import { applyPatch, checkIdentity, differences, extract, git, gitBytes, reproduce, snapshotSource, sourceDirectory, sourceEntries, treeEntries, type Source } from "./vendor";
...
function replaceWork(session: string, source: Source, current: string) {
  rmSync(work(session), { recursive: true, force: true });
  snapshotSource(source, current, work(session));
}
...
async function start(source: Source, root: string, storage: string, target: string, archive: string, adopt: boolean) {
  ...
  checkIdentity(source, sourceDirectory(root, source));
  sourceEntries(source, sourceDirectory(root, source));
  ...
  try {
    ...
    if (!adopt && differences(treeEntries(work(stage)), sourceEntries(source, sourceDirectory(root, source))).length) throw new Error("vendor source changed while starting the session; retry");
...
export async function runSession(mode: string, source: Source, root: string, storage: string, target?: string, archive?: string, adopt = false): Promise<void> {
  ...
  if (mode === "finish") {
    ...
    const desired = sourceEntries(source, current);
    ...
    replaceWork(session, source, current);
